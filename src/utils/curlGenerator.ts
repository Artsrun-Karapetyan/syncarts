import type { Collection, Environment, EnvironmentVariable, TabData } from '../contexts/WorkspaceContext';
import { applyPathVariables } from './pathVariables';
import { interpolateVariables, resolveRequestAuth } from '../contexts/workspace/requestHelpers';
...
export function generateCurlCommand(args: CurlGeneratorArgs) {
  ...
  const lines = ['curl --location ' + shellQuote(url || 'https://api.example.com')];
...
function buildHeaders(
  ...
  if (authType === 'bearer' && bearerToken) {
    headers.set('authorization', ['Authorization', `Bearer ${interpolate(bearerToken)}`]);
  }

  return Array.from(headers.values());
}

function buildBodyLines(request: TabData, interpolate: (text: string) => string) {
  ...
  const formData = (request.formData || []).filter(item => item.enabled && item.key.trim());
  if (bodyType === 'x-www-form-urlencoded') {
    return formData.map(item => `--data-urlencode ${shellQuote(`${interpolate(item.key)}=${interpolate(item.value)}`)}`);
  }

  return formData.flatMap((item) => {
    const key = interpolate(item.key);
    if (item.type === 'file') {
      return (item.files || []).map(filePath => `--form ${shellQuote(`${key}=@${filePath}`)}`);
    }
    return [`--form ${shellQuote(`${key}=${interpolate(item.value)}`)}`];
  });
}

function shouldSkipGeneratedHeader(key: string, value: string, bodyType?: string) {

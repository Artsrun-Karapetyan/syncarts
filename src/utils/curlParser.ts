...
export function parseCurlCommand(curlCommand: string): Partial<TabData> | null {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (arg === "-X" || arg === "--request") {
      ...
      setHeaderIfMissing(
        headers,
        "content-type",
        "multipart/form-data; boundary=----SyncartsCurlFormBoundary",
      );
      if (!hasExplicitMethod && method === "GET") method = "POST";
  ...
  return {
    ...
    method,
    headers:
      headers.length > 0 ? headers : [{ key: "", value: "", enabled: true }],
...
function decodeEscape(input: string, index: number) {
  ...
  if (next && map[next] !== undefined)
    return { value: map[next], nextIndex: index + 2 };
...
function setHeaderIfMissing(headers: HeaderItem[], key: string, value: string) {
  if (
    !headers.some((header) => header.key.toLowerCase() === key.toLowerCase())
  ) {

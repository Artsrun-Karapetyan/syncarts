...
import { pullForkCollection, getWorkspaceCollections } from "./collectionPullHelpers";
import { api } from "@/lib/api";

describe("collectionPullHelpers extra cases", () => {
  test("getWorkspaceCollections variations", () => {
    expect(getWorkspaceCollections({ data: { collections: [{ id: "c1" }] } } as any)[0].id).toBe("c1");
    expect(getWorkspaceCollections({ collections: [{ id: "c2" }] } as any)[0].id).toBe("c2");
    expect(getWorkspaceCollections({} as any)).toEqual([]);
  });

  test("pullForkCollection throws if not a fork", async () => {
    const update = mock();
    expect(pullForkCollection({
  ...
  test("pullForkCollection executes pull", async () => {
    ...
    await pullForkCollection({
      ...
      workspaces: [{ id: "w1", collections: [{ id: "c1", fork: { originalWorkspaceId: "w2", originalCollectionId: "origC1" } }] }] as any[]
    });

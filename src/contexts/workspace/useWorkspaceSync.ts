...
export function useWorkspaceSync(args: WorkspaceSyncArgs) {
  ...
  const removeUnavailableRemoteWorkspaces = (
    items: Workspace[],
  ...
  const reloadWorkspaces = async () => {
    try {
      ...
      setWorkspaces((prevLocals) => {
        const nextLocals = [
          ...removeUnavailableRemoteWorkspaces(prevLocals, remoteIds),
        ];
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res: any) => {
        ...
        console.log(
          "[SYNC] Fetched remote workspaces:",
          remoteWorkspaces.map((w: any) => ({ id: w.id, name: w.name })),
        );
        ...
        setWorkspaces((prevLocals) => {
          ...
          if (nextLocals.length !== prevLocals.length) {
            console.log(
              "[SYNC] Removed unavailable locals:",
              prevLocals
                .filter((p) => !nextLocals.some((n) => n.id === p.id))
                .map((w) => w.id),
            );
            hasChanges = true;
          }
...
function mergeInitialRemoteWorkspace(
  remote: any,
  ...
  if (localSignature !== remoteSignature) {
    console.log(
      "[SYNC] Signature mismatch for",
      local.id,
      "pulling remote workspace.",
    );
    nextLocals[localIndex] = remoteWorkspace;
    lastSyncedSignaturesRef.current[remote.id] = remoteSignature;
    return true;
...
function mergePolledRemoteWorkspace(
  remote: any,
  nextLocals: Workspace[],
  hasChanges: boolean,
  args: WorkspaceSyncArgs,

import type { Environment, EnvironmentVariable, Workspace } from './types';

interface EnvironmentActionsArgs {
  activeEnvironmentId: string | null;
  activeWorkspaceId: string;
  setActiveEnvIdByWorkspace: (value: Record<string, string | null> | ((prev: Record<string, string | null>) => Record<string, string | null>)) => void;
...
export function useEnvironmentActions(args: EnvironmentActionsArgs) {
  ...
  const updateGlobalVariables = (variables: EnvironmentVariable[]) => {
    updateWorkspaces(prev => prev.map(w => {
      if (w.id === activeWorkspaceId) return { ...w, globalVariables: variables };
      return w;
    }));
  };

  return { setActiveEnvironmentId, createEnvironment, updateEnvironment, deleteEnvironment, updateGlobalVariables };
}

...
function removeRequestFromItems(
  items: (Folder | SavedRequest)[],
  requestId: string,
): (Folder | SavedRequest)[] {
  return items
...
function deleteExampleFromItems(
  items: (Folder | SavedRequest)[],
  requestId: string,
  exampleId: string,
): (Folder | SavedRequest)[] {
  return items.map((item) => {
    ...
    if (item.type === "request" && item.id === requestId && item.examples) {
      return {
        ...item,
        examples: item.examples.filter((e) => e.id !== exampleId),
      };
    }
    return item;
  });

import type { Environment, EnvironmentVariable, HttpResponse, TabData, TestResult } from './types';
  ...
  const api = {
    ...
    environment: {
      set: (key: string, value: string) => {
        ...
        if (env) updateEnvironment(env.id, { variables: upsertVariable(env.variables, key, value) });
      },
      get: (key: string) => {
        if (!activeEnvironmentId || activeEnvironmentId === 'none') return undefined;
        return environments.find(e => e.id === activeEnvironmentId)?.variables.find(v => v.key === key)?.value;
      },
      unset: (key: string) => {
        if (!activeEnvironmentId || activeEnvironmentId === 'none') return;
        const env = environments.find(e => e.id === activeEnvironmentId);
        if (env) updateEnvironment(env.id, { variables: env.variables.filter(v => v.key !== key) });
      }
    },
    collectionVariables: {
      set: (key: string, value: string) => {
        replaceDraftVariables(collectionVariablesDraft, upsertVariable(collectionVariablesDraft, key, value));
      },
      get: (key: string) => collectionVariablesDraft.find(v => v.key === key)?.value,
      unset: (key: string) => {
        replaceDraftVariables(collectionVariablesDraft, collectionVariablesDraft.filter(v => v.key !== key));
      }
    },
    globals: {
      set: (key: string, value: string) => updateGlobalVariables(upsertVariable(globalVariables, key, value)),
  ...
  return {
    ...
    to: {
      have: {
        ...
        header: (key: string) => { if (!headers.has(key)) throw new Error(`Header ${key} not found`); }
      }
    }
  };
}
...
function createRequestHeadersApi(requestDraft: TabData) {
  return {
    add: (header: { key: string; value: string } | string, value?: string) => {
      const next = typeof header === 'string' ? { key: header, value: value || '' } : header;
      requestDraft.headers = [...requestDraft.headers, next];
    },
    upsert: (header: { key: string; value: string } | string, value?: string) => {
      const next = typeof header === 'string' ? { key: header, value: value || '' } : header;
      const index = requestDraft.headers.findIndex(h => h.key.toLowerCase() === next.key.toLowerCase());
      requestDraft.headers = index === -1
  ...
  return {
    ...
    set: (key: string, value: string) => {
      if (activeEnvironmentId && activeEnvironmentId !== 'none') {
        const env = environments.find(e => e.id === activeEnvironmentId);
        if (env) {
          updateEnvironment(env.id, { variables: upsertVariable(env.variables, key, value) });
          return;
        }
      }
...
function upsertVariable(variables: EnvironmentVariable[], key: string, value: string) {
  const existingIndex = variables.findIndex(v => v.key === key);
  const newVars = [...variables];
  if (existingIndex >= 0) newVars[existingIndex] = { ...newVars[existingIndex], value, enabled: true };
  else newVars.push({ id: crypto.randomUUID(), key, value, enabled: true });
  return newVars;
}

function replaceDraftVariables(target: EnvironmentVariable[], next: EnvironmentVariable[]) {
  target.splice(0, target.length, ...next);
}

async function sendRequest(
  request: string | { url: string; method?: string; headers?: Record<string, string>; body?: BodyInit | null },
  callback?: (err: unknown, response?: ReturnType<typeof createFetchScriptResponse>) => void
) {
  try {
    const url = typeof request === 'string' ? request : request.url;
    const init = typeof request === 'string' ? undefined : { method: request.method, headers: request.headers, body: request.body };
...
function createFetchScriptResponse(response: Response, body: string) {
  const headers = createHeadersReader(Object.fromEntries(response.headers.entries()));
  return {
    ...
    to: {
      have: {
        status: (code: number | string) => {
          if (typeof code === 'number' && response.status !== code) throw new Error(`Expected status ${code} but got ${response.status}`);
          if (typeof code === 'string' && response.statusText !== code) throw new Error(`Expected status to match ${code}`);
        },
        body: (text: string) => { if (body !== text) throw new Error(`Expected body to be ${text}`); },
        header: (key: string) => { if (!headers.has(key)) throw new Error(`Header ${key} not found`); }
      }
    }
  };
...
function createHeadersReader(headers: Record<string, string>) {
  return {
    ...
    has: (key: string) => Object.keys(headers).some(headerKey => headerKey.toLowerCase() === key.toLowerCase())
  };
}

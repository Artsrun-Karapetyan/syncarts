        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {testResults.map((test) => (
              <div
                key={`${test.name}-${test.passed}-${test.error ?? ""}`}
                className="response-test-row"
              >
                <div
                  style={{
                    color: test.passed
                ...
                >
                  ...
                  {!test.passed && test.error && (
                    <div className="response-test-error">{test.error}</div>
                  )}
                </div>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="response-console-output">
            {consoleLogs.map((log, idx) => {
              const isError =
                log.includes("[ERROR]") || log.includes("[SCRIPT ERROR]");

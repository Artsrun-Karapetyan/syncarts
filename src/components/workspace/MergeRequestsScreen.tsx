...
import { ArrowLeft, Ban, GitMerge, GitPullRequest } from "lucide-react";
    ...
    >
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        >
          {selectedMr ? (
            <div
              style={{ padding: 40, width: "100%", boxSizing: "border-box" }}
            >
              <div
                style={{
                  display: "flex",
                  ...
                  >
                    ...
                    {!sourceCollection && !error ? (
                      ...
                      (() => {
                        ...
                        const sourceFlat = flattenItems(
                          sourceCollection.items || [],
                        );

                        const added = sourceFlat.filter(
                          (s) => !targetFlat.find((t) => t.id === s.id),
                        );
                        const deleted = targetFlat.filter(
                          (t) => !sourceFlat.find((s) => s.id === t.id),
                        );
                        const modified = sourceFlat
                          .map((s) => {
                            ...
                            const isDiff =
                              JSON.stringify(sCompare) !==
                              JSON.stringify(tCompare);
                            if (isDiff) {
                              const changedKeys = Object.keys(sCompare).filter(
                                (key) =>
                                  key !== "changedKeys" &&
                                  JSON.stringify(sCompare[key]) !==
                                    JSON.stringify(tCompare[key]),
                              );

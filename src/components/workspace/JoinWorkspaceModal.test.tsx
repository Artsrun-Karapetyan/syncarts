...
describe("JoinWorkspaceModal", () => {
  ...
  test("enables continue button when input is provided", () => {
    render(<JoinWorkspaceModal isOpen={true} onClose={mock()} />);
    const input = screen.getByPlaceholderText("e.g. syncarts://invite/xyz... or just xyz...");
  ...
  test("handles form submission with token", async () => {
    ...
    expect(onClose).toHaveBeenCalled();
    expect(mockNavigate).toHaveBeenCalledWith({ to: "/invite/$token", params: { token: "mytoken123" } });
  });
  ...
  test("extracts token from full link", async () => {
    ...
    const input = screen.getByPlaceholderText("e.g. syncarts://invite/xyz... or just xyz...");
    fireEvent.change(input, { target: { value: "syncarts://invite/my-invite-token" } });
    
    const form = input.closest("form")!;
    fireEvent.submit(form);
    
    expect(onClose).toHaveBeenCalled();
    expect(mockNavigate).toHaveBeenCalledWith({ to: "/invite/$token", params: { token: "my-invite-token" } });
  });
  ...
  test("does not submit if input is empty", async () => {
    ...
    const form = screen.getByPlaceholderText(/e\.g\. syncarts/).closest("form")!;
    fireEvent.submit(form);

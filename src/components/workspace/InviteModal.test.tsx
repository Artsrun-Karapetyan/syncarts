...
mock.module("@/contexts/WorkspaceContext", () => ({
  useWorkspace: () => ({
    workspaces: [
      { id: "ws1", name: "Workspace 1", ownerId: "u1", collections: [], environments: [] },
      { id: "ws2", name: "Workspace 2", ownerId: "u2", collections: [], environments: [] },
      { id: "local-default", name: "Local Default", ownerId: "local", collections: [], environments: [] },
    ],
...
mock.module("@/components/workspace/invite/InviteWorkspaceSelector", () => ({
  InviteWorkspaceSelector: ({ selectedWorkspaceIds, setSelectedWorkspaceIds }: any) => (
    <div data-testid="workspace-selector">
      <button onClick={() => setSelectedWorkspaceIds(["ws2"])}>Select WS2</button>
      <button onClick={() => setSelectedWorkspaceIds(["local-default"])}>Select Local</button>
      <button onClick={() => setSelectedWorkspaceIds([])}>Clear Selection</button>
...
mock.module("@/components/workspace/invite/InviteEmailForm", () => ({
  InviteEmailForm: ({ email, setEmail, onSubmit }: any) => (
    <form data-testid="email-form" onSubmit={onSubmit}>
      <input value={email} onChange={(e) => setEmail(e.target.value)} data-testid="email-input" />
...
mock.module("@/components/workspace/invite/InviteMembersList", () => ({
  InviteMembersList: ({ onRemoveMember, onChangeRole }: any) => (
    <div data-testid="members-list">
      ...
      <button onClick={() => onChangeRole("ws1", "user1", "admin")}>Change Role</button>
    </div>
  )
}));
...
describe("InviteModal", () => {
  ...
  test("handleGenerateLink fails without selection", async () => {
    ...
    expect(screen.getByTestId("status-msg").textContent).toBe("Select at least one workspace");
  });
  ...
  test("handleGenerateLink success", async () => {
    ...
    await waitFor(() => {
      expect(mockPost).toHaveBeenCalledWith("/invites/generate", expect.any(Object));
      expect(mockReloadWorkspaces).toHaveBeenCalled();
      expect(screen.getByTestId("link-section").textContent).toContain("syncarts://invite/token123");
    });
  });
  ...
  test("handleInviteEmail success", async () => {
    ...
    await waitFor(() => {
      expect(mockPost).toHaveBeenCalledWith("/invites/add-member", expect.any(Object));
      expect(mockReloadWorkspaces).toHaveBeenCalled();
      expect(screen.getByTestId("status-msg").textContent).toBe("Member added successfully");
    });
  });
  ...
  test("handleRemoveMember success", async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId("status-msg").textContent).toBe("Member removed");
    });
  });
  ...
  test("handleChangeRole success", async () => {
    ...
    await waitFor(() => {
      expect(mockPut).toHaveBeenCalledWith("/workspaces/ws1/members/user1/role", { role: "admin" });

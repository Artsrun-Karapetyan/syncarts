          ...
          {variables.map((v) => (
            ...
            >
              <div style={{ width: 40, display: 'flex', justifyContent: 'center' }}>
                <button
                  style={{ background: 'none', border: 'none', cursor: 'pointer', color: v.enabled !== false ? 'var(--accent-primary)' : 'var(--text-tertiary)' }}
                  onClick={() => updateVariable(v.id, { enabled: v.enabled === false })}
                  title={v.enabled === false ? 'Enable variable' : 'Disable variable'}
                >

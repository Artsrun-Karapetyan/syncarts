...
export function getDynamicVariables(): VariableSuggestion[] {
  return [
    {
      key: "$guid",
      value: resolveDynamicVariable("$guid") || "Generated UUID",
      source: "Dynamic",
    },
    {
      key: "$timestamp",
      value: resolveDynamicVariable("$timestamp") || "Unix timestamp",
      source: "Dynamic",
    },
    {
      key: "$isoTimestamp",
      value: resolveDynamicVariable("$isoTimestamp") || "ISO timestamp",
      source: "Dynamic",
    },
  ];
}
  ...
  if (ancestors && ancestors.length > 0) {
    for (let i = ancestors.length - 1; i >= 0; i--) {
      ...
      if (ancestor.variables) {
        const source =
          "type" in ancestor && ancestor.type === "folder"
            ? "Folder"
            : "Collection";

...
export function resolveScopedVariable(args: {
  ...
  if (envVar) return toResolved(envVar, activeEnvironment?.name || 'Environment', 'Environment');
  ...
  if (dynamicValue !== null) {
    return { exists: true, hasValue: true, value: dynamicValue, source: 'Dynamic', sourceType: 'Dynamic' };
  }
...
function toResolved(variable: EnvironmentVariable, source: string, sourceType: ResolvedVariable['sourceType']): ResolvedVariable {
  return {
    exists: true,

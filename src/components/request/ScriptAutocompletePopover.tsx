...
export function ScriptAutocompletePopover(props: ScriptAutocompletePopoverProps) {
      ...
      {suggestions.map((suggestion, index) => (
        ...
        >
          ...
          <span style={{ minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <span className="font-mono" style={{ fontSize: 12, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
              {suggestion.label}
            </span>
          ...
          <span className="font-mono" style={{ fontSize: 10, color: 'var(--text-secondary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
function KindBadge({ kind }: { kind: ScriptSuggestion['kind'] }) {
  const color = kind === 'method' || kind === 'function'
    ? '#7dd3fc'
    : kind === 'class'
      ? '#c084fc'
      : '#93c5fd';
  const label = kind === 'function' ? 'F' : kind === 'method' ? 'M' : kind === 'class' ? 'C' : 'P';

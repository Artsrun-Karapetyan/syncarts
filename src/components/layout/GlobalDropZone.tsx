...
export function GlobalDropZone({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      ...
      {isDragging && (
        <div style={{
          position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(10, 25, 20, 0.75)',
          backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center',
          pointerEvents: 'none', transition: 'all 0.2s ease-in-out'
        }}>
          <div style={{
            position: 'absolute', inset: 24, border: '4px dashed #10b981',
            borderRadius: 24, background: 'rgba(16, 185, 129, 0.05)',
            boxShadow: '0 0 40px rgba(16, 185, 129, 0.2) inset'
          }} />
          <div className="glass-panel" style={{
            padding: '32px 64px', borderRadius: 24, display: 'flex', flexDirection: 'column',
            alignItems: 'center', gap: 16, boxShadow: '0 20px 50px rgba(0,0,0,0.5)',
            transform: 'scale(1.05)', animation: 'pulse-green 2s infinite',
            background: 'var(--bg-secondary)', border: '1px solid rgba(16, 185, 129, 0.3)'
          }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', marginBottom: 8, boxShadow: '0 4px 20px rgba(16, 185, 129, 0.4)' }}>
      ...
      <ImportModal isOpen={isModalOpen} onClose={handleCloseModal} initialFile={droppedFile} />
    </>
  );
}

...
export function SidebarToolbar({ openMrCount, onMergeRequests, onImport, onNewRequest, onNewCollection }: SidebarToolbarProps) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <ToolbarButton
          ...
          hoverBackground={openMrCount > 0 ? 'rgba(0, 240, 255, 0.2)' : 'var(--bg-tertiary)'}
        >
          {openMrCount > 0 && <MergeRequestBadge count={openMrCount} />}
        </ToolbarButton>
        <ToolbarButton tooltip="Import (or drop file anywhere)" icon={Download} onClick={onImport} />
        <ToolbarButton tooltip="New Request" icon={FilePlus2} onClick={onNewRequest} />
        <ToolbarButton tooltip="New Collection" icon={FolderPlus} onClick={onNewCollection} />
      </div>
    </div>
  );
}

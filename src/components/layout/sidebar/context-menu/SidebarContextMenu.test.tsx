...
mock.module(
  "@/components/layout/sidebar/context-menu/NewFolderMenuInput",
  () => ({
...
describe("SidebarContextMenu", () => {
  ...
  test("renders duplicate example option", () => {
    ...
    fireEvent.click(duplicateBtn);
    expect(defaultProps.duplicateExample).toHaveBeenCalledWith(
      "c1",
      "r1",
      "e1",
    );
  ...
  test("renders add example for request default status", () => {
    ...
    const addExampleBtn = screen.getByText("Add example");
    fireEvent.click(addExampleBtn);
    expect(defaultProps.addExample).toHaveBeenCalledWith(
      "c1",
      "r1",
      "New Example",
    );
  ...
  test("renders delete options correctly", () => {
    ...
    expect(defaultProps.setDeleteTarget).toHaveBeenCalledWith({
      id: "c1",
      type: "collection",
    });
  ...
  test("handles fork and pull options", async () => {
    const forkCol = {
      ...
      name: "Fork",
      type: "collection" as const,
      fork: { originalWorkspaceId: "w1", originalCollectionId: "oc1" },
      items: [],
      createdAt: 0,
      updatedAt: 0,
    };

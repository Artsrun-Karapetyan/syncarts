...
describe("sidebarDragStyles", () => {
  ...
  test("returns base when not dragging and not drop target", () => {
    ...
    expect(
      dragRowStyle({
        base,
        entity: entity1,
        draggingEntity: entity2,
        dropTarget: { ...entity2, position: "before" },
      }),
    ).toEqual(base);
  });

  test("returns lowered opacity when dragging", () => {
    const style = dragRowStyle({
      base,
      entity: entity1,
      draggingEntity: entity1,
      dropTarget: null,
    });
  ...
  test("returns box shadow when drop target inside", () => {
    const style = dragRowStyle({
      ...
      entity: entity1,
      draggingEntity: entity2,
      dropTarget: { ...entity1, position: "inside" },
    });
  ...
  test("returns box shadow when drop target before", () => {
    const style = dragRowStyle({
      ...
      entity: entity1,
      draggingEntity: entity2,
      dropTarget: { ...entity1, position: "before" },
    });
  ...
  test("returns box shadow when drop target after", () => {
    const style = dragRowStyle({
      ...
      entity: entity1,
      draggingEntity: entity2,
      dropTarget: { ...entity1, position: "after" },
    });
    expect(style.boxShadow).toBe("inset 0 -2px 0 var(--accent-primary)");
  });
});

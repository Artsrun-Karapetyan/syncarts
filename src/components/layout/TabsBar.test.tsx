...
mock.module("@/components/layout/TabsBarContextMenu", () => ({
  TabsBarContextMenu: (props: any) => (
    <div
      data-testid="tabs-bar-context-menu"
      data-ctxmenu={JSON.stringify(props.ctxMenu)}
    />
  ),
}));
...
describe("TabsBar", () => {
  ...
  test("renders dirty dot and changes opacity", () => {
    mockUseWorkspace.isTabDirty.mockImplementation(
      (tab: any) => tab.id === "tab1",
    );
    const { container } = render(<TabsBar {...defaultProps} />);
  ...
  test("handles drag and drop flow", () => {
    ...
    expect(mockUseWorkspace.moveTab).toHaveBeenCalledWith(
      "tab1",
      "tab2",
      "after",
    );
  ...
  test("handles wheel scrolling", () => {
    render(<TabsBar {...defaultProps} />);
    const scrollContainer = screen.getByTitle("New Tab").parentElement
